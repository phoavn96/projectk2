    ...
    <section id="data-thumb-view" class="data-thumb-view-header">
        ...
        <div class="table-responsive">
            ...
            <table class="table data-thumb-view">
                ...
                <tbody>
                @foreach($list as $categories_list)
                    <tr>
                        <td class="border-top-0 px-2 py-4">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input product-checkbox"
                                       value="{{$categories_list->id}}" id="checkbox_{{$categories_list->id}}">
                                <label class="custom-control-label"
                                       for="checkbox_{{$categories_list->id}}"></label>
                            </div>
                        </td>
                        <td class="categories-name" id="name">{{$categories_list->name}}</td>
                        <td class="description">{!!$categories_list->description!!}</td>
                        <td>
                <span class="text-ellipsis">
                 <?php
                    /** @var TYPE_NAME $categories_list */
                    if ($categories_list->status == 1) {
                    ?>
                     <a href="{{URL::to('/unactive-categories/'.$categories_list->id)}}">
                     <span class="fa-thumb-styling fa fa-check"></span></a>
                     <?php
                    }else{
                    ?>
                     <a href="{{URL::to('/active-categories/'.$categories_list->id)}}"><span
                    ...
                    ?>
                        ...
                        <td class="category-action">
                            <a href="{{URL::to('/admin/categories/'.$categories_list->id.'/edit')}}"
                               class="active styling-edit" ui-toggle-class="">
                                <i class="fa fa-pencil-square-o text-success text-active"></i>
                            </a>
                            <span><a class="feather icon-trash" onclick="delete_categories_id({{$categories_list->id}})"
        ...
        <div class="add-new-data-sidebar">
            <form role="form" action="{{URL::to('/admin/categories')}}" method="post" id="categories-form">
                {{csrf_field()}}
...
@section('page-script')
    ...
    <script>
        function delete_categories_id(id) {
            Swal.fire({
