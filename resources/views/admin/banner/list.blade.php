    ...
    <section id="data-thumb-view" class="data-thumb-view-header">
        ...
        <div class="table-responsive">
            <table class="table data-thumb-view">
                ...
                <tbody>
                @foreach($list as $banner_list)
                    <tr>
                        <td class="border-top-0 px-2 py-4">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input banner-checkbox"
                                       value="{{$banner_list->id}}" id="checkbox_{{$banner_list->id}}">
                                <label class="custom-control-label"
                                       for="checkbox_{{$banner_list->id}}"></label>
                            </div>
                        </td>
                        <td class="banner-name" id="name">{{$banner_list->name}}</td>
                        <td class="banner-desc">
                            <?php
                            /** @var TYPE_NAME $banner_list */echo strip_tags($banner_list->description) ?></td>
                        <td class="banner-images"><img style="width: 100px;" src="{{$banner_list->images}}"></td>
                         <td>
                            <form  action="/admin/banner" class="text-ellipsis" method="get" >
                                @csrf
                                @if($banner_list->status == 1)
                                    <input  value="{{$banner_list->id}}"  type="hidden" name="id" class="form-control">
                                    ...
                                    <input  value="{{$banner_list->status}}"  type="hidden" name="status" class="form-control"  >
                                @else
                                    <input  value="{{$banner_list->id}}"  type="hidden" name="id" class="form-control"  style="visibility: hidden;">
                        ...
                        <td class="product-action">
                            <a href="{{URL::to('admin/banner/'.$banner_list->id.'/edit')}}"
                               class="active styling-edit" ui-toggle-class="">
                                <i class="fa fa-pencil-square-o text-success text-active"></i>
                            </a>
                            <span><a class="feather icon-trash" onclick="delete_banner_id({{$banner_list->id}})"
...
@section('page-script')
    ...
    <script>
        ...
        $('#checkAll').click(function () {
            ...
            $('.banner-checkbox').prop('checked', $(this).prop('checked'));
        });
    ...
    <script> function delete_banner_id(id) {
            Swal.fire({
                title: 'Bạn có chắc chắn muốn xóa không?',
                text: "Bạn sẽ không thể hoàn nguyên thao tác này!",
                icon: 'warning',
