        ...
        if (el.length > 0) {
            el.slider({
                ...
                slide: function(event, ui) {
                    ...
                    min.text( String(values[0]).replace(/(.)(?=(\d{3})+$)/g,'$1,')+' VNĐ');
                    max.text( String(values[1]).replace(/(.)(?=(\d{3})+$)/g,'$1,')+' VNĐ');
                    $('input#price_min').attr('value',values[0]);
                    $('input#price_max').attr('value',values[1]);
                }
            });
